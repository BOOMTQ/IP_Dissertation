...
\begin{tabular}{@{}|c|c|c|c|c|c|c|@{}}
\toprule
\textbf{Strategy Name} & \textbf{Average Conversation} & \textbf{Median Conversation} & \textbf{Average Duration (h)} & \textbf{Median Duration (h)} & \textbf{Max Conversation} & \textbf{Max Duration (h)} \\ 
\midrule
...
From the table, it can be seen that the median duration of the investigation is 0 hours. This is because the calculation of duration is based on the timestamp difference between the last email from the scammer and the first email in the email interaction. Combined with the data shown in Table \ref{tab:table4}, we know that most investigators ended their strategies early on, and usually these conversations had no follow-up after replying to the scammer's first template email or automatic reply, thus not wasting any of the scammer's time.
\\

Comparing the data in Table \ref{tab:table5}, it can be found that Newbie and Impatient Consumer significantly outperform Investigator and Bargainer in all metrics. These higher values indicate that they can communicate with scammers for a longer time, wasting more of the scammer's time, thereby more effectively disrupting scam activities. In addition, the Investigator had 35 dialogues with the scammer in a key interaction that lasted 552.52 hours, indicating that the investigator strategy may perform exceptionally well when communicating with some particularly professional and patient scammers. Meanwhile, although the Bargainer exceeds the Investigator in both average and median duration, it does not exhibit particularly outstanding performance, making it the most mediocre strategy.
\\

Considering that Newbies and Impatient Consumers perform similarly on key figures, I further applied the U-test to calculate the p-value of the number of conversations and duration between these two strategies to determine whether there is a statistical difference between them. Based on the method proposed by Mann and Whitney in 1947 \cite{mann_test_1947}, the U-test involves ranking two independent samples and calculating the sum of ranks, then using these sums to estimate the U-statistic, and finally finding the p-value in the standard normal distribution table through the Z value.
\\

Considering the complexity and potential for error in manually calculating the U-test, I used the \texttt{mannwhitneyu} function \cite{noauthor_scipystatsmannwhitneyu_nodate} integrated in Python to simplify the calculation, which resulted in a p-value of 0.302 for the number of dialogues and a p-value of 0.397 for the duration. These p-values are higher than the commonly used significance threshold of 0.05, suggesting no statistically significant difference between Newbies and Impatient Consumers regarding the number and duration of dialogues. Nevertheless, Table \ref{tab:table4} has revealed that Newbie participated more in long-term dialogues, and according to Table 5, Newbie also performed better in terms of the number of dialogues and median dialogue duration. Hence, after considering various factors, Newbie is evaluated as the most effective reply strategy.
